...
2. If you only need provider-level rule discovery, call `StudioServiceHost` `GET /rules` to inspect canonical provider names and current rule summaries across known providers.
...
`StudioServiceHost` rule discovery is intentionally read-only in this work package. Continue to use `RulesWorkbench` for editing and saving validated rules.
