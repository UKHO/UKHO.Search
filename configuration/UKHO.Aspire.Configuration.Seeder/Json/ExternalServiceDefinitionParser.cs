...
namespace UKHO.Aspire.Configuration.Seeder.Json
{
    internal static class ExternalServiceDefinitionParser
    {
        private static readonly Regex _schemeRegex =
            new(@"^(?<scheme>https?)://", RegexOptions.IgnoreCase | RegexOptions.Compiled);

        private static readonly Regex _placeholderRegex = new("{{(.*?)}}", RegexOptions.Compiled);

        public static async Task<List<ExternalServiceDefinition>> ParseAndResolveAsync(AddsEnvironment environment,
            string json)
        {
            ...
            foreach (var serviceProp in envObject.EnumerateObject())
            {
                ...
                if (!serviceObject.TryGetProperty("endpoints", out var endpointsArray) ||
                ...
                var clientId = clientIdElement.GetString() ??
                               throw new InvalidDataException($"clientId for '{serviceName}' is null");
                ...
                {
                    var template = endpoint.GetProperty("url").GetString() ??
                                   throw new InvalidDataException($"Missing 'url' in endpoint of '{serviceName}'");
                    var tag = endpoint.GetProperty("tag").GetString() ?? "";

                    if (string.IsNullOrWhiteSpace(tag)) hasDefaultTag = true;
                    ...
                    if (environment == AddsEnvironment.Local && placeholderMatches.Count > 1)
                        throw new InvalidDataException(
                            $"Template for service '{serviceName}' contains more than one placeholder, which is not supported: {template}");
                ...
                if (!hasDefaultTag)
                    throw new InvalidDataException(
                        $"Service '{serviceName}' must have at least one endpoint with tag=\"\" (the default).");
        ...
        private static Task<string> LookupEndpointAsync(string scheme, string serviceName)
        {
            ...
            if (string.IsNullOrEmpty(resolvedEndpoint))
                throw new InvalidOperationException(
                    $"Cannot find reference for service {serviceName} and scheme {scheme} in the environment. Did you reference it in the Aspire startup?");
