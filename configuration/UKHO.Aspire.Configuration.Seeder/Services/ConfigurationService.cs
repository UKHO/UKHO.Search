...
namespace UKHO.Aspire.Configuration.Seeder.Services
{
    ...
    {
        public async Task SeedConfigurationAsync(AddsEnvironment environment, ConfigurationClient configurationClient,
            string serviceName, string configFilePath, string servicesFilePath, CancellationToken cancellationToken)
        {
            var label = serviceName.ToLowerInvariant();

            var configurationSetting =
                new ConfigurationSetting(WellKnownConfigurationName.ReloadSentinelKey,
                    "change this value to reload all", label) { ContentType = MediaTypeNames.Text.Plain };
            ...
            var externalServices =
                await ExternalServiceDefinitionParser.ParseAndResolveAsync(environment, externalServiceJsonCleaned);
            ...
            foreach (var externalService in externalServices)
            {
                ...
                configurationSetting = new ConfigurationSetting(key, json, label)
