...
{
    public partial class ConfigurationSettingRepository(IDbCommandFactory commandFactory, IConfigurationSettingFactory configurationSettingFactory, IDbConnectionFactory connectionFactory, ILogger<ConfigurationSettingRepository> logger, IDbParameterFactory parameterFactory) : IConfigurationSettingRepository
    {
        ...
        public async IAsyncEnumerable<ConfigurationSetting> Get(string key = KeyFilter.Any, string label = LabelFilter.Any, DateTimeOffset? moment = default, [EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            ...
            if (key is not KeyFilter.Any)
            {
                ...
                {
                    var match = TrailingWildcardRegex()
                        .Match(keys[i]);
            ...
            {
                var labels = UnescapedCommaRegex()
                             .Split(label)
                             .Select(s => s.Unescape())
                             .ToList();
            ...
            {
                yield return configurationSettingFactory.Create(reader.GetString(0), reader.GetString(1), DateTimeOffset.Parse(reader.GetString(5), styles: DateTimeStyles.AssumeUniversal), reader.GetBoolean(6), reader.IsDBNull(2) ? null : reader.GetString(2), reader.IsDBNull(3) ? null : reader.GetString(3), reader.IsDBNull(4) ? null : reader.GetString(4),
        ...
        private async Task ExecuteNonQuery(string text, IEnumerable<DbParameter> parameters, CancellationToken cancellationToken = default)
        {
        ...
        private async IAsyncEnumerable<DbDataReader> ExecuteReader(string text, IEnumerable<DbParameter> parameters, [EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
