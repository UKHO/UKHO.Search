    ...
    public class SqliteDbParameterFactory : IDbParameterFactory
    {
        public DbParameter Create<TValue>(string name, TValue? value)
        {
            return value switch
            {
                ...
                DateTime d => new SqliteParameter(name, SqliteType.Text)
                    { Value = d.ToUniversalTime().ToString("yyyy-MM-dd HH:mm:ss") },
                DateTimeOffset d => new SqliteParameter(name, SqliteType.Text)
                    { Value = d.UtcDateTime.ToString("yyyy-MM-dd HH:mm:ss") },
