...
namespace UKHO.Aspire.Configuration.Hosting
{
    ...
    {
        public static IResourceBuilder<AzureAppConfigurationResource> AddConfiguration(
            this IDistributedApplicationBuilder builder, string configurationName,
            IResourceBuilder<ParameterResource> addsEnvironment,
            IEnumerable<IResourceBuilder<ProjectResource>> configurationAwareProjects)
        {
        ...
        public static IResourceBuilder<ProjectResource> AddConfigurationEmulator(
            this IDistributedApplicationBuilder builder, string serviceName,
            IEnumerable<IResourceBuilder<ProjectResource>> configurationAwareProjects,
            IEnumerable<IResourceBuilder<ProjectResource>> externalServiceMocks, string configJsonPath,
            string externalServicesPath)
        {
            ...
            var externalServicesResolvedPath =
                Path.GetFullPath(externalServicesPath, builder.Environment.ContentRootPath);
            var externalServicesFilePath = CopyToTempFile(externalServicesResolvedPath);

            var emulator = builder
                .AddProject<UKHO_Aspire_Configuration_Emulator>(WellKnownConfigurationName.ConfigurationServiceName)
            ...
            var seederService = builder
                .AddProject<UKHO_Aspire_Configuration_Seeder>(WellKnownConfigurationName.ConfigurationSeederName)
                .WithReference(emulator)
                .WaitFor(emulator)
                .WithEnvironment(x =>
                {
                    x.EnvironmentVariables.Add(WellKnownConfigurationName.AddsEnvironmentName,
                        AddsEnvironment.Local.Value);

                    x.EnvironmentVariables.Add(WellKnownConfigurationName.ConfigurationFilePath, configFilePath);
                    x.EnvironmentVariables.Add(WellKnownConfigurationName.ExternalServicesFilePath,
                        externalServicesFilePath);
                    x.EnvironmentVariables.Add(WellKnownConfigurationName.ServiceName, serviceName);
                });

            foreach (var mock in externalServiceMocks) seederService.WithReference(mock);
