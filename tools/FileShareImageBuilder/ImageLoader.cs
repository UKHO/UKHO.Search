...
namespace FileShareImageBuilder
{
    public sealed class ImageLoader
    {
        public async Task LoadAsync(CancellationToken cancellationToken = default)
        {
            ...
            var previousImageId = await TryGetImageIdAsync(imageName, cancellationToken)
            ...
            await RunDockerAsync($"load -i \"{tarPath}\"", cancellationToken)
                .ConfigureAwait(false);
            ...
            var loadedImageId = await TryGetImageIdAsync(imageName, cancellationToken)
                .ConfigureAwait(false);
            if (!string.IsNullOrWhiteSpace(previousImageId) && !string.IsNullOrWhiteSpace(loadedImageId) && !string.Equals(previousImageId, loadedImageId, StringComparison.OrdinalIgnoreCase))
            {
                Console.WriteLine($"[ImageLoader] Removing previous docker image. ImageId={previousImageId}");
                await RunDockerAsync($"image rm -f {previousImageId}", cancellationToken)
                    .ConfigureAwait(false);
            }
