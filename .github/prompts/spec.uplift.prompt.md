...
## Objective
Update the current specification documents for the next iteration of the system by deriving deltas from the existing baseline specifications using an interactive Q&A with the requestor. Do not inspect the repository or code in this mode. Capture only what changed, was added, removed, or deprecated. All uplift outputs MUST follow the Functional Specification Document (FSD) style defined in the repository `spec-template_v1.1.md` template.
...
1. Baseline spec files under the active Work Package folder in `dev/work-packages/` (current files for: system overview, architecture components, domain specs, API functional, frontend functional, infra/deployment).
2. Initial intent/goal statement for the next iteration (requestor�s prompt).
...
- Suggest considerations the requestor may have missed (e.g., auth impacts, data migrations) and flag important decisions.
...
## Output Documents (Updated Baseline Files)
Update the existing unversioned spec files in place as needed:
- `spec-system-overview.md`
- `spec-architecture-components.md` (if necessary)
- `spec-domain-[context].md` (one per domain context as applicable)
...
- Mark sections with �No change from the current baseline� when applicable (do not restate unchanged content).

## Document Structure Template (FSD style with delta annotations)
Follow `spec-template_v1.1.md` exactly. Populate only deltas per section; otherwise write �No change from the current baseline�.
...
2. System Overview
- State deltas to capabilities/components; else �No change from the current baseline�.

3. Architecture Overview
- Use the architecture table from the template; indicate only changed cells or add �No change from the current baseline�.
...
- Use the NFR table format; show only changed/added items. For unchanged categories, write �No change from the current baseline�.
...
- Update the baseline docs in place under the active Work Package folder in `dev/work-packages/`.
- Use concise bullet lists; avoid redundancy; prefer delta statements and �No change from the current baseline�.
...
- All uplift spec files updated under the active Work Package folder in `dev/work-packages/`.
- Each uplift doc includes a Delta Summary table in the Executive Summary.
- Sections conform to `spec-template_v1.1.md` with delta annotations or explicit �No change from the current baseline�.
