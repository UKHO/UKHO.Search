...
**Target output path:** `./dev/work-packages/mvp/042-best-effort-ingestion/plan.md`
...
- [x] Work Item 3: Deployment/config documentation update for `ingestionmode` - Completed
  ...
  - [x] Task 3.1: Update work package docs - Completed
    ...
    - [x] Step 2: Linked to the existing Aspire/AppHost work package that wires the env var (`dev/work-packages/mvp/039-ingestion-mode/*`).
  - **Files**:
    - `dev/work-packages/mvp/042-best-effort-ingestion/plan.md`: Add final operator notes.
...
- If `ingestionmode` is **missing** or **invalid**, the host will **fail fast** at startup (consistent with the existing behavior introduced in `dev/work-packages/mvp/039-ingestion-mode/*`).
...
- See: `dev/work-packages/mvp/039-ingestion-mode/spec.md` and `dev/work-packages/mvp/039-ingestion-mode/plan.md`.

  - **Summary (Work Item 3)**:
    - `dev/work-packages/mvp/042-best-effort-ingestion/plan.md`: documented how to configure `ingestionmode`, supported values, default/fail-fast behavior, and Aspire references.

---

# Architecture

**Target output path:** `./dev/work-packages/mvp/042-best-effort-ingestion/architecture.md`
