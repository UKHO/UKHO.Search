...
Supersedes: `dev/work-packages/mvp/007-enrichment/archive/spec-ingestion-enrichment-core_v0.05.md`

## Change Log
- v0.06: Defined enrichment retry policy (option 2) and specified configuration keys/values to be stored under `ingestion:` alongside existing index retry settings.
...
### 8.3 Transient classification
The enrichment node must define how it identifies “transient” failures. The initial classification rule set should be minimal and explicit, e.g.:
- `OperationCanceledException` due to cancellation is not treated as transient.
- Timeouts and known transient network exceptions may be treated as transient.

(Exact exception list is implementation-defined but must be documented and covered by tests.)
