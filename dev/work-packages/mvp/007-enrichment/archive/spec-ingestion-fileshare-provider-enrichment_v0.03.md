...
Supersedes: `dev/work-packages/mvp/007-enrichment/archive/spec-ingestion-fileshare-provider-enrichment_v0.02.md`

## Change Log
- v0.03: Confirmed the context payload name as `IngestionPipelineContext` and updated provider pipeline wiring accordingly.
...
- The file share dispatch step must emit an `IngestionPipelineContext` payload that includes both:
...
- The enrichment node input must be the `IngestionPipelineContext` stream.
