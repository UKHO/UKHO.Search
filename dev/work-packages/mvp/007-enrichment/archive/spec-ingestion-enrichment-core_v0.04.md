...
Supersedes: `dev/work-packages/mvp/007-enrichment/archive/spec-ingestion-enrichment-core_v0.03.md`

## Change Log
- v0.04: Confirmed `IngestionPipelineContext` resides in the `UKHO.Search.Ingestion.Pipeline` namespace and must be implemented as a single public type in its own `.cs` file.
...
- The enrichment node must route the failed item to the index dead-letter path, recording a pipeline error with category `Transform`.
...
## 9. Open questions
- Confirm whether the enrichment node should also add a breadcrumb / structured error details into envelope context for diagnostics.
