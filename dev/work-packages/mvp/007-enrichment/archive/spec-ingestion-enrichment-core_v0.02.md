...
Version: v0.02  
Status: Draft  
Work Package: `dev/work-packages/mvp/007-enrichment/`  
Supersedes: `dev/work-packages/mvp/007-enrichment/archive/spec-ingestion-enrichment-core_v0.01.md`

## Change Log
- v0.02: Adopted the selected approach to carry `IngestionRequest` + `CanonicalDocument` via a dedicated pipeline context payload (option 3).
...
- A core context payload type that ensures enrichers can access both the original `IngestionRequest` and the `CanonicalDocument`.
...
## 6. Core pipeline context payload (selected approach)
### 6.1 Problem
...
### 6.3 Proposed type
Create a new public record (name TBD in implementation) in `UKHO.Search.Ingestion` such as:
...
- The payload must preserve all existing envelope metadata (key/message id/attempt/context).

## 7. Core pipeline node
### 7.1 Responsibility
Add a core node that:
- Accepts a stream of context envelopes (payload includes both request + operation)
...
- Unit tests for the context payload flow:
  - Dispatch produces context containing both request + operation
  - Enrichment outputs the operation with unchanged envelope metadata

## 9. Open questions
- Confirm the final name and namespace for the new context payload type.
