...
**Target output path:** `dev/work-packages/mvp/015-canonical-document-uplift/plan-ingestion-canonical-document-uplift_v0.01.md`

**Based on:** `dev/work-packages/mvp/015-canonical-document-uplift/spec-canonical-document-uplift_v0.01.md`
...
- [x] Work Item 4: Verify ingestion rules DSL parsing/evaluation unaffected and update tests/docs if required - Completed
  ...
  - **Acceptance Criteria**:
    ...
    - `wiki/Ingestion-Rules.md` is reviewed and only updated if necessary to correct misleading content (do not add BasicEnricher details proactively).
  - **Definition of Done**:
    - Tests pass
  ...
  - [x] Task 4.2: Documentation review - Completed
    - [x] Step 1: Review `wiki/Ingestion-Rules.md` for references to `CanonicalDocument.Source` or indexing of `source`.
  ...
  - **Files**:
    ...
    - `wiki/Ingestion-Rules.md`: only if required.
  - **Work Item Dependencies**: Work Item 1.
  - **Run / Verification Instructions**:
    - `dotnet test`
  - **Completed Summary**:
    ...
    - Note: the then-referenced ingestion rules guide was not present in this repository workspace, so no documentation update was applied.
...
- Keep documentation changes minimal and corrective only (especially for `wiki/Ingestion-Rules.md`).
