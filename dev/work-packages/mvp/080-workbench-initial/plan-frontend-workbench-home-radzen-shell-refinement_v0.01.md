...
**Target output path:** `dev/work-packages/mvp/080-workbench-initial/plan-frontend-workbench-home-radzen-shell-refinement_v0.01.md`

**Based on:** `dev/work-packages/mvp/080-workbench-initial/spec-frontend-workbench-home-radzen-mockup_v0.01.md`
...
- The existing completed plan documents in `dev/work-packages/mvp/080-workbench-initial` remain historical records and must not be overwritten.
...
- [x] Work Item 3: Populate the upper middle tabs with common Radzen form and data components using sample data - Completed
  ...
  - **Completion Summary**:
    ...
    - Refined `src/Workbench/server/WorkbenchHost/Components/Pages/Home.razor` and `Home.razor.css` again to remove the bespoke shell color system, switch the page to a stock Radzen Material theme selector, and keep the CSS focused on layout and sizing so the rendered appearance stays as close as possible to plain Radzen.
    - Moved the active `RadzenTheme` host to `src/Workbench/server/WorkbenchHost/Components/App.razor`, enabled interactive head updates, and wired the showcase theme selectors through the shared `ThemeService` so changing between `material` and `material-dark` now updates the live page theme instead of only changing local component state.
    - Polished the remaining sidebar and tab demo surfaces by replacing the explorer list with a `RadzenTree`, converting search chips to `RadzenBadge`, converting outline summaries to `RadzenCard`, and switching the showcase confirmation message to `RadzenAlert` so the temporary shell stays closer to Radzen-native components throughout.
    - Corrected the live Radzen theme-switching implementation by moving the active `RadzenTheme` host back into the interactive `Home.razor` page head content while keeping `HeadOutlet` interactive in `App.razor`, ensuring the selected stock theme now updates in the same interactive render boundary as the theme selector.
    - Aligned the shell surfaces with the active Radzen theme by switching the demo cards to filled Radzen card variants and by binding the shell background/text styling to Radzen theme variables, so the stock `material-dark` selection now produces dark surfaces and light text across the page instead of leaving large white HTML regions untouched.
    - Replaced the remaining raw left and right sidebar tab-strip buttons with `RadzenTabs` bound directly to the active sidebar panel state, keeping the activity bars synchronized while removing the last custom non-Radzen tab control from the shell chrome.
    - No new automated tests were added, per specification.
