...
Supersedes: `spec-domain-rule-discovery_v0.01.md`
Source discovery document: `dev/work-packages/mvp/044-rule-discovery/spec-domain-rule-discovery_v0.01.md`
...
This document extends the discovery evidence from `dev/work-packages/mvp/044-rule-discovery/spec-domain-rule-discovery_v0.01.md` with draft mapping proposals for later ingestion rule authoring.
...
Selected discovery document version/path: `dev/work-packages/mvp/044-rule-discovery/spec-domain-rule-discovery_v0.01.md`
...
- searchable value is weak; later domain review may decide to exclude this class
...
- no searchable rule proposed
- this class should likely be ignored until better metadata exists
...
- later domain review may decide to exclude this class from search
...
- no searchable rule proposed unless support assets are intentionally indexed
...
- no searchable rule proposed unless domain owners want support packs indexed
...
| minorVersion | `toInt($path:properties["update number"])` | update number is version-like |
| category | `data product` | individual s100 product batch |
| series | `s100` | safest stable series label without direct `product code` |
| instance | `$path:properties["product name"]` | strongest identity-bearing field |
| searchText | `s100 product $path:properties["product name"] from $path:properties["producing agency"] edition $path:properties["edition number"] update $path:properties["update number"]` | semantic summary |
...
- later rule authors may choose to normalize `product identifier` from `s-101` to `s101` and use that as `series`
...
- if readme files are not meant for search, this class should be excluded
...
- if these assets stay api-only and hidden from ui, a search rule may still be unnecessary
...
- `printtoorder` has rich metadata but the supplied domain description says it is not searchable; this is the largest policy decision affecting later rule authoring
- several low-value support/business-continuity/file-sharing units (`addssupport`, `paperproducts`, `var`, `chersoft`, some `avcs-bespokeexchangesets` content) have technically mappable metadata but weak user-facing search value
...
1. should `printtoorder` remain completely excluded from search even though its metadata is rich enough for high-quality search documents?
2. should support/readme/startup-pack classes in `adds`, `adds-s57`, `var`, and `avcs-bespokeexchangesets` be indexed at all, or should rules only target user-facing datasets?
3. for `adds-s100` class `2`, should later rule authoring normalize `product identifier` values such as `s-101` into canonical series values such as `s101`?
4. should `adsd-viewerupdates` remain non-searchable because files are not shown on the ui, or should api consumers still benefit from indexing?
